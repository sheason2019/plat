...
pub fn tar(config_path: PathBuf, output_path: PathBuf) -> anyhow::Result<()> {
    ...
    // 读取 Plugin 配置
    let config_path = config_path.join("plugin.json");
    let config_bytes = fs::read(&config_path)?;
    let config: PluginConfig = serde_json::from_slice(&config_bytes)?;

    // 写入 WASM 文件
    tar.append_path_with_name(config_path.join(&config.wasm_root), "plugin.wasm")?;

    // 写入静态资源文件夹
    let assets_dir = config_path.join(&config.assets_root);
